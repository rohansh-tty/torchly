...
class BasicBlock(nn.Module):
    ...
    def __init__(self, config, in_planes, planes, stride=1):
        ...
        self.bn1 = getattr(torch.nn, config.channel_norm)(1,planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                               stride=1, padding=1, bias=False)
        # self.bn2 = nn.BatchNorm2d(planes)
        self.bn2 = getattr(torch.nn, config.channel_norm)(1,planes)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                ...
                # self.bn1 = getattr(torch.nn, config.channel_norm)(**config.channel_norm_params[config.channel_norm],planes)
            )
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, config, num_classes=10):
        ...
        self.bn1 = getattr(torch.nn, config.channel_norm)(1,64)
